...
A **subagent-as-tool demo coordinator** — a tutor-role BaseAgent that does no calculus computation itself, instead delegating every symbolic operation to a registered peer agent (`calculus_specialist`, configured to point at the deployed `calculus-agent` route). Demonstrates the `subagents:` config block and the framework-supplied `delegate_to_agent` tool shipped in fipsagents 0.22.0 (agent-template PR #173).

The agent loop here is intentionally minimal — most of the differentiation lives in `agent.yaml` (the `subagents:` block) and `prompts/system.md` (strict-delegation tutor prompt). The Python class in `src/agent.py` is ~15 lines.

**Important contrasts with `../calculus-agent/`:**

- This is **hand-built**, not a `fips-agents create` scaffolded output. Bootstrapped by copying `../calculus-agent/`'s scaffold and editing.
- Uses **subagents** (peer-agent delegation), not **MCP tools**. The `mcp_servers:` block is empty; the `subagents:` block is populated.
