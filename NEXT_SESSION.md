...
serving, CLI tools, and registry.

The documented reference model is now **`ibm-granite/granite-3.3-8b-instruct`**.
Granite was chosen over Gemma 4 because vLLM + Gemma 4 in FIPS mode is
still pending an upstream PR, and this tutorial targets `fips-agents` —
many users will be deploying to FIPS-enabled clusters. Granite is known
to work in FIPS mode today.

**Granite has not yet been smoke-tested end-to-end against this
tutorial.** The plan is to spin Granite back up on the `mcp-rhoai`
cluster (which previously had it deployed) and run a quick verification.
...
tool's `variables` array. Both are model issues, not tutorial issues. With
the switch to Granite 3.3 8B (which has stronger tool-calling), those
specific failure modes are expected to disappear — but that's the
hypothesis the smoke test on `mcp-rhoai` needs to confirm.

## Remaining work

1. **Granite smoke test on `mcp-rhoai`** -- spin Granite 3.3 8B back up on
   the `mcp-rhoai` cluster (it was previously deployed there), point the
   tutorial stack at it, and run the eval cases. Confirm that the
   tool-calling quirks observed under Gemma 4 don't reappear and that the
   end-to-end calculus flow works.

2. **Human walkthrough** -- a manual read-through by someone who hasn't
...
3. **End-to-end re-verification on a fresh cluster** -- the prerequisites
...
4. **Namespace cleanup (optional)** -- the `calculus-agent` and
