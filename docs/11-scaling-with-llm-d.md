...
!!! warning "OpenShift caveat for the upstream quickstart"
    The upstream `optimized-baseline` overlay's `vllm/vllm-openai` image expects to write to `/.config` (vLLM usage stats) and `/.triton` (Triton compiler cache) at filesystem root. OpenShift's default `restricted-v2` SCC runs the container as an arbitrary high UID with no permissions on root-owned directories, so the model server pod CrashLoopBackOffs at engine init with `PermissionError: '/.triton'`. Tracked upstream as [llm-d/llm-d#1430](https://github.com/llm-d/llm-d/issues/1430); fix is mounting `emptyDir` volumes at both paths. Until the upstream overlay carries the fix, the patch lives at `retrospectives/2026-05-06_tutorial-walkthrough/manifests/llm-d-modelserver/patch-decode.yaml` in this repo.
